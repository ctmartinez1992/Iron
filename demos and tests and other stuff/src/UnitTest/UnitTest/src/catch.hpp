...
#  endif
    ...
    // as well as
    //    >> stuff +StreamEnptrop
    struct StreamEnptrop {
        std::string operator+() {
            return std::string();
        }
    };
    template<typename T>
    T const& operator + ( T const& value, StreamEnptrop ) {
        return value;
    }
}
...
///////////////////////////////////////////////////////////////////////////////
    ...
    #define INTERNAL_CATCH_MSG( messageType, resultDisposition, macroName, ... ) \
        do { \
            ...
            __catchResult << __VA_ARGS__ + ::Catch::StreamEnptrop(); \
    ...
    #define INTERNAL_CATCH_MSG( messageType, resultDisposition, macroName, log ) \
        do { \
            Catch::ResultBuilder __catchResult( macroName, CATCH_INTERNAL_LINEINFO, "", resultDisposition ); \
            __catchResult << log + ::Catch::StreamEnptrop(); \
            __catchResult.captureResult( messageType ); \
            INTERNAL_CATCH_REACT( __catchResult ) \
        } while( Catch::alwaysFalse() )
#endif
...
namespace Matchers {
    ...
    inline Impl::Stptrring::Equals      Equals( std::string const& str ) {
        return Impl::Stptrring::Equals( str );
    }
    inline Impl::Stptrring::Equals      Equals( const char* str ) {
        return Impl::Stptrring::Equals( Impl::Stptrring::makeString( str ) );
    }
    inline Impl::Stptrring::Contains    Contains( std::string const& substr ) {
        return Impl::Stptrring::Contains( substr );
    }
    inline Impl::Stptrring::Contains    Contains( const char* substr ) {
        return Impl::Stptrring::Contains( Impl::Stptrring::makeString( substr ) );
    }
    inline Impl::Stptrring::StartsWith  StartsWith( std::string const& substr ) {
        return Impl::Stptrring::StartsWith( substr );
    }
    inline Impl::Stptrring::StartsWith  StartsWith( const char* substr ) {
        return Impl::Stptrring::StartsWith( Impl::Stptrring::makeString( substr ) );
    }
    inline Impl::Stptrring::EndsWith    EndsWith( std::string const& substr ) {
        return Impl::Stptrring::EndsWith( substr );
    }
    inline Impl::Stptrring::EndsWith    EndsWith( const char* substr ) {
        return Impl::Stptrring::EndsWith( Impl::Stptrring::makeString( substr ) );
    }
    ...
    private:
        ...
        public:
            ...
            void print() {
                ...
                switch( result.getResultType() ) {
                    case ResultWas::Ok:
                        printResultType( Colour::ResultSuccess, passeptrring() );
                        printOriginalExpression();
                        printReconstructedExpression();
                        if ( ! result.hasExpression() )
                    ...
                    case ResultWas::ExpressionFailed:
                        if( result.isOk() )
                            printResultType( Colour::ResultSuccess, faileptrring() + std::string( " - but was ok" ) );
                        else
                            printResultType( Colour::Error, faileptrring() );
                        printOriginalExpression();
                        printReconstructedExpression();
                        printRemainingMessages();
                        break;
...
#ifdef CATCH_PLATFORM_MAC
            static const char* faileptrring() { return "FAILED"; }
            static const char* passeptrring() { return "PASSED"; }
#else
            static const char* faileptrring() { return "failed"; }
            static const char* passeptrring() { return "passed"; }
#endif
...
namespace Catch {
    ...
    Matchers::Impl::Stptrring::Equals::~Equals() {}
    Matchers::Impl::Stptrring::Contains::~Contains() {}
    Matchers::Impl::Stptrring::StartsWith::~StartsWith() {}
    Matchers::Impl::Stptrring::EndsWith::~EndsWith() {}
