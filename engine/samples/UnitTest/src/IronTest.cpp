...
SCENARIO("Clamp feature of the Vector2 object.", "[Vector2]") {
	GIVEN("5 initialized Vector2 objects with the following values: (400, 2.95) : (-5153.1, 56) : (400, 2.95) : (400, 2.95) : (400, 2.95). 1 pointer to a Vector2 object to store results.") {
		...
		iron::Vector2* dst = new iron::Vector2();

		WHEN("The method clamp(Vector2, float, float, float, float, Vector2*) is called with the following parameters: (v1, 0, 10, 500, 20, dst).") {
			iron::Vector2::clamp(v1, 0, 10, 500, 20, dst);

			THEN("\"dst\" values will be set to 400 and 10.") {
				REQUIRE(dst->x == 400);
				REQUIRE(dst->y == 10);
			}
		}

		WHEN("The method clamp(Vector2, float, float, float, float, Vector2*) is called with the following parameters: (v2, 5, 5, 6, 50, dst).") {
			iron::Vector2::clamp(v2, 5, 5, 6, 50, dst);

			THEN("\"dst\" values will be set to 5 and 50.") {
				REQUIRE(dst->x == 5);
				REQUIRE(dst->y == 50);
			}
		}

		WHEN("The method clamp(Vector2, Vector2, Vector2, Vector2*) is called with the following parameters: (v2, Vector2(0, 0), Vector2(1, 1), dst).") {
			iron::Vector2::clamp(v3, iron::Vector2(0, 0), iron::Vector2(1, 1), dst);

			THEN("\"dst\" values will be set to 0.5 and 0.5.") {
				REQUIRE(dst->x == 0.5f);
				REQUIRE(dst->y == 0.5f);
			}
		}
